...
const { isBlocked, moveEntity } = require('./occupancy');
...
function applyCommand(room, cmd) {
  ...
  switch (cmd.type) {
    case 'move': {
      ...
      const nx = cx + delta[0];
      const ny = cy + delta[1];
      const lvl0 = p.currentLocation?.level ?? 0;
      if (!isBlocked(room, lvl0, nx, ny)) {
        ...
        // keep occupancy index in sync
        moveEntity(room, p, p.lastLocation, p.currentLocation);
      } else {
        try { console.log('[DEBUG server] blocked', { userId: cmd.userId, dir, at: [nx, ny, lvl0] }); } catch (_) {}
