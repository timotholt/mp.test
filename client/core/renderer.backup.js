...
export async function setupAsciiRenderer() {
  try {
    ...
    // --- BEGIN: Enhanced compositor test harness (40x20 demo) ---
    // Builds three layers: floor (.), walls (# border), and a blue '@' entity at center.
    // This exercises the new per-layer occlusion control and alpha compositing.
    try {
      const W = 40, H = 20;

      // Helper to make a repeated char row
      const rowOf = (ch) => ch.repeat(W);

      // Floor: fill with '.' everywhere
      const floorMap = Array.from({ length: H }, () => rowOf('.')).join('\n');

      // Walls: only draw border '#'; interior is space to avoid overdraw
      const wallsRows = [];
      for (let y = 0; y < H; y++) {
        if (y === 0 || y === H - 1) {
          wallsRows.push(rowOf('#'));
        } else {
          wallsRows.push('#' + ' '.repeat(W - 2) + '#');
        }
      }
      const wallsMap = wallsRows.join('\n');

      // Entity: single '@' at the center, spaces elsewhere
      const cx = Math.floor(W / 2);
      const cy = Math.floor(H / 2);
      const entityRows = [];
      for (let y = 0; y < H; y++) {
        if (y === cy) {
          entityRows.push(' '.repeat(cx) + '@' + ' '.repeat(W - cx - 1));
        } else {
          entityRows.push(rowOf(' '));
        }
      }
      const entityMap = entityRows.join('\n');

      // Character color mapping: gray floor, lighter gray walls, blue entity
      const charColorMap = JSON.stringify({
        '.': [0.40, 0.40, 0.40],
        '#': [0.60, 0.60, 0.60],
        '@': [0.20, 0.55, 1.00],
      });

      // Apply colors globally (renderer packs them into asciiViewTexture on each update)
      try { rc.surface && rc.surface.setCharacterColorMap(charColorMap); } catch (_) {}

      // Compose layers in order; floors should NOT write to occlusion; walls/entities SHOULD.
      const layers = [
        { mapString: floorMap, writeToOcclusionBuffer: false },
        { mapString: wallsMap, writeToOcclusionBuffer: true },
        { mapString: entityMap, writeToOcclusionBuffer: true },
      ];

      // Enable enhanced mode and route RC's dungeonPass calls through our layers by default.
      rc.enhanced = true;
      rc.__testLayers = layers;
      const _origDungeonPass = rc.dungeonPass.bind(rc);
      rc.dungeonPass = (maybeLayers) => _origDungeonPass(maybeLayers || rc.__testLayers);

      // Kick a frame to draw the composed room; RC's own loop will continue to use our wrapper.
      try { rc.renderPass && rc.renderPass(rc.__testLayers); } catch (_) { rc.renderPass && rc.renderPass(); }
    } catch (e) { console.warn('[enhanced test] failed to install test harness', e); }
    // --- END: Enhanced compositor test harness ---
