...
// - Persistence: grimDark namespaced storage via core/localStorage.js with key 'vol:' + groupId
...
export function getValue(groupId = 'MASTER', defaultValue = 1) {
  ...
  try {
    // Prefer namespaced storage
    const saved = parseFloat(LS.getItem('vol:' + gid));
...
export function setValue(groupId = 'MASTER', v, opts = {}) {
  ...
  // Persist via namespaced storage helper
  try { LS.setItem('vol:' + gid, String(val)); } catch (_) {}
  if (!silent) {
